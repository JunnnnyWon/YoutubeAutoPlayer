import tkinter        # 무채색 테마
        ...
        } import ttk, messagebox, filedialog
import json
import os
...
class ModernVideoScheduler:
    def __init__(self, root):
        ...
        self.colors = {
            ...
            'primary_dark': '#e55a2e', # 다크 오렌지
            'secondary': '#ff8e53',    # 세컨더리 오렌지
            'accent': '#ffb08a',       # 액센트 오렌지
            'gradient_start': '#ff6b35',
            'gradient_end': '#ff8e53',
            'bg_primary': '#f8fafc',   # 라이트 그레이 배경
            'bg_secondary': '#ffffff', # 화이트 카드
            'bg_hover': '#f1f5f9',     # 호버 배경
            'text_primary': '#1e293b', # 다크 텍스트
            'text_secondary': '#64748b', # 그레이 텍스트
            'text_muted': '#94a3b8',   # 뮤티드 텍스트
            'border': '#e2e8f0',       # 라이트 보더
            'shadow': '#f0f0f0',       # 그림자
    ...
    def on_closing(self):
        ...
        try:
            # 임시 등록된 폰트 제거
            if hasattr(self, 'loaded_font_paths'):
                import ctypes
    ...
    def load_jamsil_fonts(self):
        """잠실체 폰트 로드 - 실제 폰트 이름 감지"""
        try:
            font_folder = r"c:\Users\a0109\Desktop\Codes\The_Jamsil_OTF_2024"
            
            # fonttools로 실제 폰트 이름 추출
            try:
                from fontTools.ttLib import TTFont
                font_name_detection = True
            except ImportError:
                print("⚠️ fonttools가 없어서 기본 이름을 사용합니다")
                font_name_detection = False
            
            # Windows API를 사용하여 폰트 등록
            import ctypes
            ...
            font_files = [
                ("The Jamsil OTF 1 Thin.otf", "thin"),
                ("The Jamsil OTF 2 Light.otf", "light"), 
                ("The Jamsil OTF 3 Regular.otf", "regular"),
                ("The Jamsil OTF 4 Medium.otf", "medium"),
                ("The Jamsil OTF 5 Bold.otf", "bold"),
                ("The Jamsil OTF 6 ExtraBold.otf", "extrabold")
            ]
                ...
                if os.path.exists(font_path):
                    ...
                    if font_name_detection:
                        try:
                            ...
                            if font_family:
                                actual_name = font_family
                            else:
                                actual_name = f"The Jamsil {weight.title()}"
                        except Exception as e:
                            print(f"폰트 이름 추출 실패 {font_file}: {e}")
                            actual_name = f"The Jamsil {weight.title()}"
                    else:
                        # 파일명에서 추정
                        if "Thin" in font_file:
                            actual_name = "The Jamsil Thin"
                        elif "Light" in font_file:
                            actual_name = "The Jamsil Light"
                        elif "Regular" in font_file:
                            actual_name = "The Jamsil"
                        elif "Medium" in font_file:
                            actual_name = "The Jamsil Medium"
                        elif "Bold" in font_file and "Extra" not in font_file:
                            actual_name = "The Jamsil Bold"
                        elif "ExtraBold" in font_file:
                            actual_name = "The Jamsil ExtraBold"
                        else:
                            actual_name = "The Jamsil"
                    ...
                    if result > 0:
                        loaded_fonts[weight] = actual_name
                        self.loaded_font_paths.append(font_path)
                        print(f"✅ {weight}: {actual_name} 로드 성공")
                    else:
                        print(f"❌ {weight}: {actual_name} 로드 실패")
            ...
            if loaded_fonts:
                self.jamsil_fonts = loaded_fonts
                print(f"🎉 잠실체 폰트 {len(loaded_fonts)}개 성공적으로 로드됨")
                
                # 폰트 테스트
                self.test_font_availability()
            else:
                raise Exception("폰트 로드 실패")
                
        except Exception as e:
            print(f"⚠️ 폰트 로드 실패: {e}")
            print("🔄 맑은 고딕으로 대체합니다 (가장 안정적)")
            # 맑은 고딕의 다양한 굵기 사용
            self.jamsil_fonts = {
                'thin': 'Malgun Gothic',
                'light': 'Malgun Gothic', 
                'regular': 'Malgun Gothic',
                'medium': 'Malgun Gothic',
                'bold': 'Malgun Gothic',
                'extrabold': 'Malgun Gothic'
            }
    ...
    def test_font_availability(self):
        ...
        try:
            ...
            for weight, font_name in self.jamsil_fonts.items():
                try:
                    ...
                    if font_name.lower() in actual_family.lower():
                        print(f"✅ {weight}: {font_name} 사용 가능")
                    else:
                        print(f"⚠️ {weight}: {font_name} -> {actual_family}로 대체됨")
                except Exception as e:
                    print(f"❌ {weight}: {font_name} 테스트 실패 - {e}")
    ...
    def setup_styles(self):
        ...
        style.configure('Modern.TCombobox',
                       ...
                       font=(self.jamsil_fonts['regular'], 10))
    ...
    def create_header(self):
        ...
        title_label = tk.Label(left_frame, 
                              ...
                              font=(self.jamsil_fonts['medium'], 32, 'bold'),
        ...
        self.time_label = tk.Label(time_frame,
                                  ...
                                  font=(self.jamsil_fonts['bold'], 18),
        ...
        date_label = tk.Label(time_frame,
                             ...
                             font=(self.jamsil_fonts['light'], 9),
                             bg=self.colors['bg_secondary'],
    ...
    def add_shadow(self, widget):
        """위젯에 그림자 효과 추가 (시뮬레이션)"""
        shadow = tk.Frame(widget.master, bg=self.colors['card_shadow'], height=2)
        shadow.place(in_=widget, x=2, y=2, width=widget.winfo_reqwidth(), 
    ...
    def create_modern_settings_panel(self, parent):
        ...
        panel_container.pack(side='left', fill='y', padx=(0, 25))
        ...
        title_label = tk.Label(title_frame, text="스케줄 설정",
                              font=(self.jamsil_fonts['medium'], 16),
                              bg=self.colors['primary'], fg='white')
    ...
    def create_time_section(self, parent):
        ...
        section_title = tk.Label(section_frame, text="⏰ 시간 설정",
                                font=(self.jamsil_fonts['medium'], 14),
                                bg=self.colors['bg_secondary'],
                                fg=self.colors['text_primary'])
        ...
        start_label = tk.Label(start_group, text="시작 시간",
                              font=(self.jamsil_fonts['regular'], 11),
                              bg=self.colors['bg_secondary'],
                              fg=self.colors['text_primary'])
        ...
        colon1 = tk.Label(start_time_frame, text=":",
                         font=(self.jamsil_fonts['regular'], 14),
                         bg=self.colors['bg_secondary'],
                         fg=self.colors['text_primary'])
    ...
    def create_media_section(self, parent):
        ...
        self.media_path = tk.Entry(path_frame, font=(self.jamsil_fonts['regular'], 11),
                                  relief='flat', bd=0,
                                  borderwidth=2,
    ...
    def create_action_buttons(self, parent):
        ...
        add_btn = tk.Button(button_frame, text="➕ 스케줄 추가",
                           ...
                           font=(self.jamsil_fonts['medium'], 12),
                           cursor='hand2',
    ...
    def create_modern_schedule_view(self, parent):
        ...
        schedule_container.pack(side='right', fill='both', expand=True)
        ...
        status_label = tk.Label(header_content, text="📊 총 0개의 스케줄",
                               font=(self.jamsil_fonts['light'], 11),
                               bg=self.colors['primary'], fg='white')
    ...
    def update_modern_schedule_display(self):
        ...
        if not schedules:
            ...
            empty_text = tk.Label(center_frame, text="아직 스케줄이 없습니다",
                                 font=(self.jamsil_fonts['regular'], 16),
                                 bg=self.colors['bg_secondary'],
                                 fg=self.colors['text_muted'])
            empty_text.pack(pady=(10, 0))
            
            empty_subtext = tk.Label(center_frame, text="왼쪽 패널에서 새 스케줄을 추가해보세요",
                                    font=(self.jamsil_fonts['light'], 12),
                                    bg=self.colors['bg_secondary'],
                                    fg=self.colors['text_muted'])
    ...
    def create_schedule_item(self, index, schedule):
        ...
        path_label = tk.Label(path_frame,
                             text=f"🔗 {path_text}",
                             font=(self.jamsil_fonts['light'], 10),
                             bg='white', fg=self.colors['text_secondary'],
